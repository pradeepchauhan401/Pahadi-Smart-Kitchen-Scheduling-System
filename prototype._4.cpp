...
//  v4: Do-while input | No preloaded orders | 5–10 min window
//      | 1.5 min fuzzy | First-order priority boost
// ============================================================
#include <iostream>
#include <vector>
#include <string>
...
public:
    ...
    virtual double calculate(double dist, double wait, double prep,
                             bool isFirstAtLocation) const = 0;
};
...
class SmartSchedulingStrategy : public IPriorityStrategy {
    ...
    // Priority boost given to the very first order placed at a location.
    // Ensures early customers aren't deprioritised by later high-distance ones.
    static constexpr double FIRST_ORDER_BOOST = 8.0;
public:
    double calculate(double dist, double wait, double prep,
                     bool isFirstAtLocation) const override {
        double score = (dist * DIST_W) + (wait * WAIT_W) - (prep * PREP_W);
        if (wait > WAIT_TH)       score *= 1.5;
        if (isFirstAtLocation)    score += FIRST_ORDER_BOOST;
        return score;
    }
};
...
private:
    ...
    // ── Variable batch window (tightened to 5–10 min) ───────
    //
    //   avgWait  = locationWaitSum[loc] / locationWaitCount[loc]
    //   window   = clamp(MIN_WIN + avgWait * WAIT_SCALE, MIN_WIN, MAX_WIN)
    //            = 5 min  when avgWait is very low
    //            = 10 min when avgWait >= ~10 min
    //
    // Fuzzy grace: ±1.5 min past the computed window.
    //
    static constexpr double MIN_WIN         =  5.0;   // tightened from 16
    static constexpr double MAX_WIN         = 10.0;   // tightened from 25
    static constexpr double WAIT_SCALE      =  0.5;
    static constexpr double FUZZY_TOLERANCE =  1.5;   // tightened from 2.0
...
public:
    ...
    void placeOrder(const string& customerName,
        ...
        if (it != activeBatches.end() &&
            ...
            // Merged orders are NOT first — no boost on recalculation
            existing.priorityScore = strategy->calculate(
                existing.distance, existing.waitTime, existing.prepTime, false);

            locationWaitSum[location]   += wait;
            locationWaitCount[location] += 1;

            double newWindow = computeWindow(location);
            ...
            // ── New batch — first order at this location gets boost ──
            ++orderCounter;
            // isFirstAtLocation = true for brand-new location entry
            bool isFirst = (activeBatches.find(location) == activeBatches.end());
            double score = strategy->calculate(dist, wait, cookTime, isFirst);
...
int main() {
    ...
    cout << "\n";
    cout << "  +----------------------------------------------+\n";
    cout << "  |      Smart Pahadi Kitchen System  v4         |\n";
    cout << "  |   Real-time | 5-10 min Window | First Boost  |\n";
    cout << "  +----------------------------------------------+\n";

    FoodFactory::printMenu();

    // ----------------------------------------------------------
    // SECTION A: Live terminal input via do-while loop
    // The loop runs at least once, then asks after each order
    // whether to add another — no upfront count needed.
    // ----------------------------------------------------------
    cout << "\n+--------------------------------------------------+\n";
    cout << "|            LIVE ORDER ENTRY                      |\n";
    cout << "+--------------------------------------------------+\n";
    ...
    // ----------------------------------------------------------
    // SECTION B: Process all orders and display dispatch table
    // ----------------------------------------------------------
